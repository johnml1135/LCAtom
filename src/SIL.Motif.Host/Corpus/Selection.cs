...
/// A selection's hash is what makes that drift detectable rather than invisible.
/// </para>
/// <para>
...
/// </remarks>
/// <param name="Name">
/// A caller-assigned label for what this selection is — typically a project name, or a project name plus a
...
/// </param>
/// <param name="Words">
/// The selection's word forms, sorted ordinally and <b>distinct</b>. Sorting makes <see cref="Sha256"/> depend
...
/// <para>
/// Deduplicating is the same argument one step further, and it was missed on the first pass. A selection is a
/// <i>set</i> of word forms: <see cref="Parser.GrammarCoverageFigure.Compute"/> compares it to what was analysed with
/// set semantics, and a word form analysed twice is analysed once. If this list kept duplicates, two selections
/// covering identically the same words would carry different hashes and a figure would report drift against
/// itself. Identity and comparison have to agree about what a selection is.
/// </para>
/// </param>
/// <param name="Sha256">
...
/// here. A grammar coverage figure citing a <see cref="Name"/> whose current hash no longer matches this
...
/// result. Optional only so that a selection extracted from the open project itself, whose origin is not in
/// question, need not restate it; **any selection from outside the project should carry one**, and a selection
...
{
    /// <summary>
    /// Whether a report over this selection may state accuracy, as opposed to reach.
    /// </summary>
    /// <remarks>
    /// A selection with no provenance cannot: an unattested selection is exactly the case where a failed analysis
    /// is ambiguous between a grammar gap, a typo, and an out-of-scope token. Reach figures are unaffected —
    /// they are what a large uncurated selection is <i>for</i>.
    /// </remarks>
    public bool SupportsAccuracyClaims => Provenance?.SupportsAccuracyClaims ?? false;

    /// <summary>
    /// The only supported way to build a <see cref="Selection"/>: deduplicates and sorts
    ...
    public static Selection Create(
        string name, IEnumerable<string> words, CorpusProvenance? provenance = null)
    {
        ...
        var ordered = words
            ...
            .ToList();
        return new Selection(name, ordered, ComputeSha256(ordered), provenance);
    }
