...
namespace SIL.LCAtom.Runner.Caching;

/// <summary>
/// Tracks whether a given <see cref="LcmCache"/> *instance* remains safe to reuse for a subsequent
/// Assess/Apply/pre-flight call, without adding any state to <see cref="LcmCache"/> itself (this
...
/// </summary>
/// <remarks>
/// 1. A rolled-back <see cref="SIL.LCAtom.Runner.Apply.ChangeSetApplier.Apply"/> whose failure
///    could plausibly have touched one of those caches — see
///    <see cref="SIL.LCAtom.Runner.Apply.RollbackCacheInvalidator"/> for why the only public,
...
/// 2. A non-committing <see cref="SIL.LCAtom.Runner.Assessment.ChangeSetAssessor.Assess"/> of an
///    operation kind flagged by
///    <see cref="SIL.LCAtom.Runner.Assessment.DerivedCachePoisoningOperationKinds"/> as possibly
///    touching one of those same caches — Assess always mutates-then-rolls-back, so running it over
...
public static class CacheReusability
{
    ...
    /// <summary>
    /// Marks <paramref name="cache"/> as no longer safe to reuse for a subsequent Assess/Apply/
    ...
    /// already poisoned, rather than letting a caller silently Assess/Apply against it and receive a
    /// possibly-different digest than a healthy cache would produce. Called at the top of both
    /// <see cref="SIL.LCAtom.Runner.Assessment.ChangeSetAssessor.Assess"/> and
    /// <see cref="SIL.LCAtom.Runner.Apply.ChangeSetApplier.Apply"/>.
    /// </summary>
