...
/// <summary>
/// Strict, closed JSON parser for a Change Set document. Unknown operation kinds and unknown
/// top-level properties (on the Change Set, an operation, or a placement) are hard errors — this
...
/// </summary>
public static class ChangeSetJsonParser
{
    private static readonly HashSet<string> ChangeSetProperties = new(StringComparer.Ordinal)
    {
        "contractVersions", "changeSetId", "requires", "operations", "extensions",
    };
    ...
    public static ChangeSetEnvelope Parse(string json)
    {
        if (json is null)
        ...
        if (root.ValueKind != JsonValueKind.Object)
            throw new ContractParseException("A Change Set must be a JSON object.");

        RejectUnknownProperties(root, ChangeSetProperties, "the Change Set");

        var contractVersions = ParseContractVersions(root);
        var changeSetId = ParseRequiredId(root, "changeSetId");
        var requires = ParseRequires(root, changeSetId);
        var operations = ParseOperations(root);

        ValidateContractVersionsCoverGroups(contractVersions, operations);

        var extensions = ParseExtensions(root, "the Change Set");

        return new ChangeSetEnvelope(contractVersions, changeSetId, requires, operations, extensions);
    }
    ...
    private static List<CanonicalId> ParseRequires(JsonElement root, CanonicalId changeSetId)
    {
        ...
        if (requiresEl.ValueKind != JsonValueKind.Array)
            throw new ContractParseException("'requires' must be an array of Change Set canonical ids.");
        ...
        foreach (var item in requiresEl.EnumerateArray())
        {
            ...
            if (requiredId.Equals(changeSetId))
                throw new ContractParseException("A Change Set cannot list its own changeSetId in 'requires'.");
        ...
        foreach (var operationEl in operationsEl.EnumerateArray())
        {
            ...
            if (!seenOperationIds.Add(operation.OperationId))
            {
                throw new ContractParseException(
                    $"Duplicate operationId '{operation.OperationId.Value}' within a Change Set. " +
                    "Operation ids must be unique within one Change Set.");
            }
            ...
            if (operation.EntityId is { } entityId && !seenEntityIds.Add(entityId))
            {
                throw new ContractParseException(
                    ...
                    "Change Set. A proposed entity id may have only one creator operation.");
            }
