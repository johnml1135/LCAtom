...
/// <summary>
/// Renders a <see cref="ChangeSetEnvelope"/>'s intent projection as (pre-canonicalization) JSON
...
/// </summary>
/// <remarks>
/// Excluded fields are simply never written: <c>changeSetId</c>, <c>rationale</c>,
...
{
    public static string WriteProjectionJson(ChangeSetEnvelope changeSet)
    {
        ...
        using (var writer = new Utf8JsonWriter(stream))
        {
            ...
            foreach (var entry in changeSet.ContractVersions)
                writer.WriteString(entry.Key, entry.Value);
            writer.WriteEndObject();

            if (changeSet.Requires.Count > 0)
            {
                writer.WriteStartArray("requires");
                foreach (var id in changeSet.Requires.OrderBy(x => x))
