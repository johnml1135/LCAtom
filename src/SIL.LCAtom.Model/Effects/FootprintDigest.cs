...
namespace SIL.LCAtom.Model.Effects;

/// <summary>
/// Computes the footprint digest: a hash of only the pre-mutation ("before") half of a set of
/// <see cref="ExpectedEffect"/>s — the live baseline an Assessment read, or Apply's pre-flight
/// re-read of that same baseline. See docs/adr/0004-prerequisite-graph-stable-ids-bound-apply.md,
/// decision 3, and <see cref="SIL.LCAtom.Model.Assessment.BoundAssessmentAnchor.FootprintDigest"/>.
/// </summary>
/// <remarks>
...
/// Assessments of the *same* baseline against *different* proposed content therefore share one
/// footprint digest even though their effect digests differ — exactly the property Apply's drift
/// check needs (it recomputes the current "before" independent of what any particular Change Set
