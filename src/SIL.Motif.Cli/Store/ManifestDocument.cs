...
public sealed class ManifestDocument
{
    ...
    /// decision 3). <c>null</c> when no dry run has been computed yet, or after an amend invalidates the prior
    /// one (it was bound to the previous content's footprint/effect digest). <c>apply</c> requires
    /// this to be present and not stale — a bare apply with no bound DryRun is a hard error.
    /// </summary>
