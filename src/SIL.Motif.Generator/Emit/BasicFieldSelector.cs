...
/// <summary>
/// Selects slice 1's exact field list from the joined model, deriving the list from the manifest and
/// the model rather than hardcoding it. Two rules, neither of
/// which hardcodes a field name:
/// </summary>
/// <remarks>
/// <list type="number">
/// <item><b>The class filter.</b> Every <c>Scope=in</c>, <c>Kind=basic</c>, <c>Verbs=set|clear</c>
/// row declared on <see cref="LexEntryAndMoFormClasses"/> — the two classes that make up this
/// slice's family (<c>lexEntry</c>'s authorable rows, "plus ...
...
/// is the pre-existing hand-written kind that must be regenerated, not because it
