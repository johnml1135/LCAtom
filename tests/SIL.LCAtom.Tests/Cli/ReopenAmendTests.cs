...
/// manifests by the frozen <c>changeSetId</c> (a movable <c>currentIntentDigest</c> pointer), exactly
...
public sealed class ReopenAmendTests : IDisposable
{
    ...
    {
        _tempRoot = Path.Combine(Path.GetTempPath(), "SIL.LCAtom.Tests.Reopen", Guid.NewGuid().ToString("N"));
    ...
    public void Commit_Reopen_Amend_KeepsId_MovesDigest_RetainsBothObjectVersions_ResetsStatusToProposed()
    {
        ...
        var changeSetId = ExtractChangeSetId(firstFinalize.Output);
        var firstDigest = ExtractIntentDigest(firstFinalize.Output);

        var store = new ChangeSetStore(_storeDir);
        var manifestPath = store.ManifestPath(changeSetId);
        Assert.Contains("\"status\": \"proposed\"", File.ReadAllText(manifestPath));
        Assert.True(File.Exists(store.ObjectPath(firstDigest)));

        // Simulate this Change Set having since been applied (a real prior status), so amend's
        ...
        // --- reopen: loads v1's content into a NEW draft carrying the SAME frozen changeSetId ---
        var reopenResult = Commands.Reopen(_storeDir, "v2", changeSetId);
        Assert.Equal(0, reopenResult.ExitCode);
        Assert.Contains(changeSetId, reopenResult.Output);
        ...
        var amendedChangeSetId = ExtractChangeSetId(amendFinalize.Output);
        var secondDigest = ExtractIntentDigest(amendFinalize.Output);

        // (1) The id is unchanged.
        Assert.Equal(changeSetId, amendedChangeSetId);
    ...
    [Fact]
    public void Reopen_UnknownChangeSetId_Fails()
    {
    ...
    private static string ExtractChangeSetId(string output)
    {
        ...
        Assert.True(end > start, $"Could not parse changeSetId from output: {output}");
