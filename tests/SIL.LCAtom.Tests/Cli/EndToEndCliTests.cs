...
namespace SIL.LCAtom.Tests.Cli;

/// <summary>
/// Stage E proof, on a real project: drives the CLI command handlers directly (never shells out to
/// the built executable) through the full <c>new -&gt; add-set-gloss -&gt; finalize -&gt; assess -&gt;
...
public sealed class EndToEndCliTests : IDisposable
{
    ...
    public EndToEndCliTests()
    {
        ...
        _tempRoot = Path.Combine(Path.GetTempPath(), "SIL.LCAtom.Tests.Cli", Guid.NewGuid().ToString("N"));
        _fwDataPath = TestLangProjFixture.CopyToTempAndGetFwDataPath(_tempRoot);
        _storeDir = Path.Combine(_tempRoot, ".lcatom-store");
    }
    ...
    [Fact]
    public void FullLoop_New_AddSetGloss_Finalize_Assess_Apply_Log_DrivesRealProjectEndToEnd()
    {
        var (senseGuid, wsTag, originalGloss) = FindSenseWithKnownGloss();
        var canonicalId = SIL.LCAtom.Contract.Ids.CanonicalId.FromGuid(senseGuid);
        ...
        const string applier = "lcatom-cli-tests";
        ...
        Assert.Contains(changeSetId, listResult.Output);
        Assert.Contains("proposed", listResult.Output);

        // --- show ---
        var showResult = Commands.Show(_storeDir, changeSetId);
        Assert.Equal(0, showResult.ExitCode);
        Assert.Contains(changeSetId, showResult.Output);
        Assert.Contains(canonicalId.Value, showResult.Output);

        // --- assess: real before/after from LibLCM, non-mutating ---
        var assessResult = Commands.Assess(_storeDir, changeSetId, _fwDataPath);
        Assert.Equal(0, assessResult.ExitCode);
        Assert.Contains(canonicalId.Value, assessResult.Output);
        Assert.Contains($"\"{originalGloss}\" -> \"{newGloss}\"", assessResult.Output);
        Assert.Contains("effectDigest: sha256:", assessResult.Output);

        // Assess must not have mutated the project: gloss unchanged when re-read from disk.
        AssertGlossOnDisk(senseGuid, wsTag, originalGloss);

        // --- apply: real commit + save ---
        var applyResult = Commands.Apply(_storeDir, changeSetId, _fwDataPath, applier);
        Assert.Equal(0, applyResult.ExitCode);
        Assert.Contains("Applied Change Set", applyResult.Output);
        ...
        Assert.Contains("1 LCAtom entry", logResult.Output);

        // --- apply again: idempotent, no duplicate log entry, no re-mutation ---
        var secondApplyResult = Commands.Apply(_storeDir, changeSetId, _fwDataPath, applier);
    ...
    public void UnknownCommands_And_MissingArguments_ReturnNonZeroWithClearErrors()
    {
        ...
        var missingChangeSet = Commands.Show(_storeDir, "agent_AAECAwQFBgcICQoLDA0ODw");
        Assert.NotEqual(0, missingChangeSet.ExitCode);
        Assert.Contains("not found", missingChangeSet.Output, StringComparison.OrdinalIgnoreCase);
    ...
    private static string ExtractChangeSetId(string finalizeOutput)
    {
        // "Finalized draft 'name' -> Change Set <id> (status: proposed)."
        const string marker = "-> Change Set ";
