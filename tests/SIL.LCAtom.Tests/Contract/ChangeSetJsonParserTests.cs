...
/// </summary>
public class ChangeSetJsonParserTests
{
    private static string MinimalValidChangeSet(string changeSetId = TestIds.ChangeSet1) => $$"""
        {
    ...
    [Fact]
    public void Parse_MinimalValidChangeSet_Succeeds()
    {
        var changeSet = ChangeSetJsonParser.Parse(MinimalValidChangeSet());

        Assert.Equal(TestIds.ChangeSet1, changeSet.ChangeSetId.Value);
        Assert.Single(changeSet.Operations);
        Assert.Empty(changeSet.Requires);
    ...
    public void Parse_RequiresArray_ParsesMultipleEntriesInAuthoredOrder()
    {
        ...
        var changeSet = ChangeSetJsonParser.Parse(json);

        Assert.Equal(2, changeSet.Requires.Count);
        // The parser preserves authored order; only digest projection sorts unordered members.
        Assert.Equal(TestIds.Right1, changeSet.Requires[0].Value);
        Assert.Equal(TestIds.Left1, changeSet.Requires[1].Value);
    }

    [Fact]
    public void Parse_RequiresEntryEqualToOwnChangeSetId_IsRejected()
    {
        var json = $$"""
            {
              "contractVersions": { "sequence": "1.0" },
              "changeSetId": "{{TestIds.ChangeSet1}}",
              "requires": ["{{TestIds.ChangeSet1}}"],
              "operations": [
        ...
        var ex = Assert.Throws<ContractParseException>(() => ChangeSetJsonParser.Parse(json));
        Assert.Contains("cannot list its own changeSetId", ex.Message);
    }
    ...
    public void Parse_DuplicateRequiresEntry_IsRejected()
    {
        ...
        Assert.Throws<ContractParseException>(() => ChangeSetJsonParser.Parse(json));
    }
    ...
    [Fact]
    public void Parse_PaddedChangeSetId_IsRejected()
    {
        var json = MinimalValidChangeSet(TestIds.ChangeSet1.Substring(0, TestIds.ChangeSet1.Length - 2) + "==");
        Assert.Throws<ContractParseException>(() => ChangeSetJsonParser.Parse(json));
    }
    ...
    public void Parse_IntegralNumberInAfterPayload_IsAccepted()
    {
        ...
        var changeSet = ChangeSetJsonParser.Parse(json);
        Assert.Equal(42, changeSet.Operations[0].After!.Value.GetProperty("someCount").GetInt32());
    }
    ...
    public void Parse_DependsOn_ParsesAndDeduplicates()
    {
        ...
        var changeSet = ChangeSetJsonParser.Parse(json);
        var dependsOn = changeSet.Operations[1].DependsOn;
    ...
    {
        Assert.Throws<ContractParseException>(() => ChangeSetJsonParser.Parse("[1,2,3]"));
    }
    ...
    public void Parse_OperationsMayBeEmpty()
    {
        ...
        var changeSet = ChangeSetJsonParser.Parse(json);
        Assert.Empty(changeSet.Operations);
    }
}
