...
- [ ] **Step 1: Write the tests** — an absent file yields documented defaults; a malformed file refuses
...
- [ ] **Step 2: Run red**
- [ ] **Step 3: Implement.** Defaults: one scope named `default`, words = all with a manual analysis, engine
  = the fast one, per-word limit 1s, regression gating **off**, purge-on-apply **on**.
- [ ] **Step 4: `motif config show --project P [--json]`**, so a caller can see the resolved configuration
...
- [ ] **Step 1: Write the migration tests** — every existing row survives; the new columns are populated for
  old rows in a way that is honest about not knowing (see below).
- [ ] **Step 2: Run red**
- [ ] **Step 3: Migrate.** `Assessments` gains `Assessor`, `Kind`, `ScopeJson`, `ScopeDigest`,
...
- [ ] **Step 4: An `AssessmentRepository`** with the interface the later tasks need: record an Assessment
  with its scope and kind, read one, list by Proposal, promote one to current.
- [ ] **Step 5: Run green and commit.** State what the compatibility floor moved to.
...
- [ ] **Step 1: Design the interface first and write it down.** An Assessor declares **which kinds it can
...
- [ ] **Step 2: Write the tests against a fake Assessor**, not against PanGloss. The seam is the test
  surface; `FakePanGloss` already exists for the process-level contract.
- [ ] **Step 3: Run red**
- [ ] **Step 4: Implement `PanGlossAssessor`** over `pangloss batch --stats --cache <path>`. Stats caches
...
- [ ] **Step 1: Write the argv test** — `motif trial <proposalId> --project P` prints a job id; the job
...
- [ ] **Step 2: Run red**
- [ ] **Step 3: Implement.** A Trial is a kick-off: **no Proposal state changes.** Reuse the existing
  single-open scratch — B32's fix must not be undone by opening the Baseline again for the parse.
- [ ] **Step 4: Run green and commit.** `dry-run` survives unchanged as the cheap question.
...
- [ ] **Step 1: Tests** — a report over a scope that did not collect what it needs refuses **naming the
...
- [ ] **Step 2: Run red**
- [ ] **Step 3: Implement** `motif report --project P --assessment A --kind <k> [--word W] [--text T]` and
...
- [ ] **Step 1: Tests** — two Assessments over different word sets compare on the intersection; a differing
...
- [ ] **Step 2: Run red**
- [ ] **Step 3: Implement** as an Assessment of the **difference** kind, stored and citable — `compare` is
...
- [ ] **Step 1: Tests first, and one of them is the trap.** Applying promotes the candidate Assessment to
...
- [ ] **Step 2: Run red**
- [ ] **Step 3: Implement.** Promotion happens **before** the sweep and the promoted Assessment is excluded
...
- [ ] **Step 1: Argv test, implement, run green and commit**
