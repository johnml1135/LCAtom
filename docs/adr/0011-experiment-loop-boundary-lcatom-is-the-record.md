...
ADR 0010 made the HermitCrab experimentation loop LCAtom's primary purpose but did not say where
LCAtom stops and the rest of the loop begins. Three documents then answered differently:

- **HC surface scope §6** claimed LCAtom "owns comparison and the verdict", and that *"did it parse
  better" = diffing gloss signatures and confirmed counts across runs — LCAtom's job*.
- **Stage-2 S5** claimed the opposite: PanGloss "is run by a thin orchestration script, **never by
  LCAtom**", and LCAtom "**stores and lists by provenance; it never interprets** tool output."
...
**LCAtom is the record. It never runs the parser and never renders a verdict.**

The loop divides like this:

```
LCAtom                         │ external infrastructure
───────────────────────────────┼──────────────────────────────
author a change set            │
assess (state delta)           │
...
### 1. LCAtom stores and displays; it never interprets
...
This does **not** delete `SIL.LCAtom.HermitCrab`. Reverse `Expand` — authoring change sets in
...
- **Attachments** are report blobs carrying a configuration-declared **label** — `"PanGloss Report"`,
  ...
  in which case the CLI pretty-prints it (`lcatom show <id> --attachment pangloss-report`).
- **Metrics** are configuration-declared typed values — `corpus-a-coverage: percent`,
  `regression-status: enum[pass,fail]`. LCAtom stores them, lists them, and shows how one moved
...
- **LCAtom evaluates nothing.** No metric gates an apply in v1. Whoever runs the loop reads the
  numbers and decides.
...
predicates over declared metrics are *the operator's policy* rather than *LCAtom's judgment* is the
...
UI, Avalonia, LexBox sync, and the cloud substrate. The boundary moved by exactly one step: LCAtom
