...
| B17 | **`MoMorphSynAnalysis.Components`/`GlossBundle` classified with low confidence** — no documented guidance found either way; defaulted to `semantic-operation`. Needs confirmation against FieldWorks usage. | open |
| B18 | **~300 of 473 in-scope rows classified by field-name heuristic**, not an explicit citation. Defensible but unverified individually. The `Rationale` column distinguishes generic from cited, so confidence is machine-visible — treat cited rows as authoritative and generic rows as reviewable. **Raised in importance by [ADR 0012](adr/0012-build-order-hc-spine-first-kinds-generated.md):** kind generation now reads these classifications directly, so a wrong row becomes a wrong shipped kind. Verify-lazily vs dedicated-audit is still undecided. | open |
| B19 | **Construct-to-kind-segment naming is not mechanical.** `LexSense.Gloss` carries `Construct=lexSense`, but the shipped kind is `lexical/sense/setGloss` — segment `sense`, not `lexSense`. The generator ([ADR 0012](adr/0012-build-order-hc-spine-first-kinds-generated.md)) cannot run unattended until the manifest's construct names are normalized or a mapping is committed. | open |
| B20 | **17 authorable rows carry a multi-construct string** (`possibility\|partOfSpeech\|lexRefType\|…`) with no single kind namespace. Needs a resolution rule — most likely fan-out to one kind per construct — before generation is unambiguous. Generator prerequisite alongside B19. | open |
| B21 | **L0's object-creation closure is uncomputed.** [ADR 0012](adr/0012-build-order-hc-spine-first-kinds-generated.md) defines L0 by manifest query (`HcReachable=yes`, `Group!=grammar`, 37 fields), but a field being *read* by `HCLoader` does not mean an object can be *created* with only those fields set. LibLCM factories and model invariants will require fields the parser never reads, and the manifest has no `required` column to derive them from. L0's true scope is unknown until this is computed. | open |
