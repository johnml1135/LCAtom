...
| B18 | **~300 of 473 in-scope rows classified by field-name heuristic**, not an explicit citation. Defensible but unverified individually. The `Rationale` column distinguishes generic from cited, so confidence is machine-visible — treat cited rows as authoritative and generic rows as reviewable. | open |
