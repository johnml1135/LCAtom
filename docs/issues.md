...
| B13 | **Cross-process protocol unspecified.** Framing, error/exit-code contract, and one-shot-vs-daemon for the Python/Rust consumers. A hybrid was recommended (store ops one-shot; project ops via a per-project daemon that *is* the exclusive-write guarantee). **Reshaped 2026-07-27, still open.** Two constraints are now fixed: (a) [ADR 0012](adr/0012-build-order-hc-spine-first-kinds-generated.md)'s 915 generated kinds make typed CLI verbs impossible, so a generic change-set-JSON path is forced, not optional; (b) [ADR 0011](adr/0011-experiment-loop-boundary-lcatom-is-the-record.md)'s scratch-copy export weakens the daemon's *exclusive-write* rationale, since experiments touch throwaway copies. Its *cache-reuse* rationale strengthened instead — measured cache load is **10.1s cold / 3.6s warm on a 61-entry project**, against **0.05s** to copy the whole 43 MB project, and 7 of the 12 verbs need no cache at all. | open |
...
| B22 | **Reverse `Expand` lost its oracle.** [ADR 0011](adr/0011-experiment-loop-boundary-lcatom-is-the-record.md) deletes forward projection, which S6 had designated `Expand`'s round-trip oracle. `Expand` must instead be validated against HC's own conformance suite (`grammar.xml` + `words.yaml`, ground truth mechanically re-derived) — a different and unbuilt harness. | open |
| B23 | **Attachment/metric configuration surface is unspecified.** [ADR 0011](adr/0011-experiment-loop-boundary-lcatom-is-the-record.md) establishes labelled attachments, Markdown render designation, and typed metrics, all configuration-declared — but the config format, whether the label taxonomy is open or closed, and the metric type vocabulary are undecided. | open |
