...
| `MOT-19` — the CLI as the full product surface, text and JSON | M2/M4 | Large, and grows with every other item | **Partly built** — `open`, Proposal `list`/`show`, Dry Run, apply, applied log, Corpus `corpora`/`show-corpus`, and their JSON forms all render shared projections; usage records command/argument shape and summarizes call counts and adjacent sequences without project data. Remaining read surfaces must follow the same projection-plus-two-renderers rule as they land; the analysis aggregate and report/check surfaces are not CLI verbs yet |
