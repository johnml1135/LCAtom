...
| `MOT-24` — per-user worker, protocol, jobs, and scheduler | M2/M4 | Large, architecture foundation | **Partly built** — the worker, named-pipe control and binary transfer, capability negotiation, database ownership/migration, project runtime, and verified minimal Baseline publication are built. Next is live-host lease integration with the per-project lane, followed by reusable Dry Runs, durable job scheduling, PanGloss limits, launcher/version selection, and the remaining recovery surfaces |
