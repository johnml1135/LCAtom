...
| `MOT-19` — the CLI as the full product surface, text and JSON | M2/M4 | Large, and grows with every other item | **Partly built** — `open`, Proposal `list`/`show`, Dry Run, apply, applied log, Corpus `corpora`/`show-corpus`, and the manual side of the analysis aggregate through `analyses` all render shared text/JSON projections; usage records command/argument shape and summarizes call counts and adjacent sequences without project data. `analyses` opens a scratch cache and never invokes PanGloss; its automatic side waits for a caller that can establish the current Corpus and grammar identities. Remaining read surfaces must follow the same projection-plus-two-renderers rule as they land; report/check surfaces are not CLI verbs yet |
