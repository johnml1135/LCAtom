...
## A collaborative laboratory for improving FieldWorks grammars

Status: proposal for discussion.

Date: 2026-07-28.

Proposed repository name: **Grammar-Workbench**  
Command-line application: **`gbench`**
...
Grammar Workbench is that application.

It has two first-class interfaces:

- native Avalonia components inside FieldWorks for linguists and native-speaker review;
- an independently runnable .NET 10 CLI, `gbench`, for automation and AI-assisted or autonomous
  investigation.

Grammar Workbench does not replace FieldWorks, LibLCM, Harmony, LexBox, or PanGloss. It defines the
...
| Software development | Grammar Workbench |
|---|---|
...
Grammar Workbench owns the collaborative evidence and review model:
...
- Grammar Workbench domain services, Harmony integration, PanGloss orchestration, and `gbench` target
  .NET 10;
...
| Cases, gold revisions, proposals and review workflow | Grammar Workbench | New bounded application context |
| Application-specific Harmony entities and changes | Grammar Workbench | Product semantics using Harmony mechanics |
| PanGloss orchestration and artifact correlation | Grammar Workbench | Joins parser facts to cases and proposals |
| AI provider integration and job policy | Grammar Workbench | Application policy, secrets, budgets and audit |
| Independent automation CLI | Grammar Workbench (`gbench`) | First-class noninteractive product interface |
| Coverage research and migration evidence | This repository as it transitions | Existing LCAtom research assets remain useful |

## Why this repository should become Grammar-Workbench

Creating a new repository was initially attractive because the application has a lifecycle distinct
from FieldWorks, PanGloss, Harmony, and LexBox. This repository can become that repository instead.
...
However, the transition must be explicit:

- **Grammar Workbench is not LCAtom renamed while retaining its original architecture.**
- ADR 0013 remains binding: Harmony is the change mechanism.
- The old LCAtom operation envelope, digest, snapshot, runner, and applied-log model do not become a
...
The repository should eventually be renamed `Grammar-Workbench`, with product and package names
chosen independently of the old LCAtom namespace.

## The `gbench` CLI

`gbench` is a separately installable .NET 10 command-line application. It is the automation and AI
...
gbench project init
gbench cases list
gbench cases show <case>
gbench assess run --baseline <revision> --candidate <revision> --suite <suite>
gbench assess compare <baseline-assessment> <candidate-assessment>
gbench review show <proposal>
gbench review propose <proposal> --decision <...>
gbench review abstain <proposal> --reason <...>
gbench ask-ai <proposal> --provider <provider> --model <model>
gbench jobs list
gbench jobs resume <job>
gbench artifacts verify <artifact>
gbench submit <proposal>
```
...
- Later `gbench apply` may request FieldWorks to apply an already approved command; FieldWorks remains
  the apply authority.
- Offline operation uses a local Harmony store and content-addressed artifact directory.
- Remote operation synchronizes through the Grammar Workbench/LexBox service boundary.
...
PanGloss never emits a “better” verdict. Grammar Workbench presents the facts and records the
reviewer's decision.

## Harmony model

Harmony supplies commits, history, snapshots, unknown-change preservation, and synchronization.
Grammar Workbench supplies the application objects and changes.
...
Ordered grammar cannot use a generic last-writer-wins order field. Grammar Workbench must define
...
grammar-specific semantics remain in Grammar Workbench.
...
```text
approved Grammar Workbench command
  ...
  → application receipt returned to Grammar Workbench
```
...
Harmony/`.fwdata` synchronization for a bounded lexical surface. Grammar Workbench should reuse its
lessons:
...
services. It does not need to absorb Grammar Workbench's domain into the lexical MiniLcm API.
...
2. Grammar Workbench materializes baseline and candidate inputs without touching the live project.
3. PanGloss assesses both against the pinned suite.
4. Grammar Workbench records reports and exact deltas.
...
1. `gbench` selects cases with complete, pinned evidence.
...
3. Grammar Workbench does not silently rewrite gold while reviewing a grammar proposal.
...
- adopt Grammar Workbench terminology;
- rename the CLI entry point to `gbench`;
- retain LCAtom research and coverage evidence;
- quarantine superseded LCAtom runtime architecture;
- establish CI and ownership;
- write the context glossary and repository-level ADR for the transition.
...
- display the result through `gbench`;
...
- allow `gbench` to submit human or AI reviews.
...
> PanGloss evidence, `gbench` review, and FieldWorks preview without mutating the live project?
...
6. One `gbench review` proposal or abstention with provenance.
7. One FieldWorks Avalonia preview using a fake or local Grammar Workbench projection.
...
| Repository rename accidentally revives LCAtom | ADR 0013 remains binding; obsolete runner is quarantined, not promoted |
| Scope expands into reimplementing FieldWorks | UI and live linguistic data stay in FieldWorks; workbench owns workflow only |

## Decisions requested

This proposal asks stakeholders to agree that:

1. this repository becomes **Grammar-Workbench**;
2. the CLI is named **`gbench`** and targets .NET 10;
...
7. Grammar Workbench owns cases, gold revisions, proposals, reviews, orchestration, and artifact
   correlation;
...
2. Confirm repository and package naming.
...
5. Establish the Grammar Workbench solution skeleton and CI.
6. Implement the evidence-only `gbench` vertical slice.
