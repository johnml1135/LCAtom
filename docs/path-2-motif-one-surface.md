...
*Research report. Scope: whether Motif, as designed and as built, can serve as the single, reliable,
...
covered by other agents and are out of scope here, except where Motif's own documents already compare
...
`[VERIFIED — FwLiteMaui.csproj:26-27]`. Motif's own internal design document reaches the identical
conclusion about itself: mobile can carry an Motif change set as an opaque, unopened payload, but
*interpreting* one — which is the entire point of "the runner gives that change set one meaning"
(`README.md:18`) — "happens where LibLCM is" `[VERIFIED — one-api-problem.md:95-97]`. So "Motif is the
...
## 1. What "Motif is the one API surface" would actually mean

Motif is not a CRUD API and does not claim to be one. It is explicit about this in its own product
...
This is a genuinely different *kind* of API from a live-editing surface, and Motif's own comparison
document says so without hedging: *"MiniLcm is nouns and verbs — `CreateEntry`, `UpdateSense`,
`DeletePartOfSpeech` — executed immediately. Motif is one noun, a change set, carrying a generated kind
...
**Direct answer:** Motif does not become the CRUD API, and the repository's own most recent design work
(dated the same day as this brief, 2026-07-27) explicitly rules that out as the goal. What Motif proposes
...
FieldWorks-desktop-only world, LibLCM itself). A reader who hears "Motif is the one API surface" and
...
## 2. How Motif reaches Android and Linux
...
**Net finding:** LibLCM genuinely runs Motif's own dependency chain (`net8.0`, the pinned TFM) on Linux
...
1. **The native ICU dependency has no Android build anywhere in evidence.** The `icu-fw` apt package is
   ...
   three repositories read for this report `[VERIFIED absence — grep across `liblcm`, `Motif`, and
...
2. **The one team that already tried this concluded it wasn't viable and built around it, not through
   ...
   Motif's path; it is the identical native-dependency wall, already hit and already routed around, by
   engineers who needed the exact same thing Motif needs (an ICU-and-LibLCM-backed model on a phone) and
   chose to leave LibLCM off the device rather than solve the porting problem.
3. **Motif's `Runner`/`Host` are architecturally the same shape as the bridge that was excluded.** Both
   are thin layers whose entire job is to hold `LcmCache` and drive LibLCM's factories and UOW machinery
   `[VERIFIED — AGENTS.md:39-40 for Motif's TFM/dependency split; `Runner`/`Host` `.csproj` files confirm
   `net8.0` + `PackageReference Include="SIL.LCModel" Version="11.0.0-beta0150"` at
   `src/SIL.Motif.Runner/SIL.Motif.Runner.csproj` and `src/SIL.Motif.Host/SIL.Motif.Host.csproj`]`.
   Nothing about Motif's own code changes the native-dependency calculus that already excluded the
...
constraints) that nobody in either repository examined has started, and it is a project Motif's own
...
ICU build — Windows or Linux, desktop or server — and Android participates as a remote client. Motif's
own most recent design document proposes exactly this, independently of this brief, as "Option B: One
ring at the *contract* level, not the method level": make Motif's change set the lingua franca, give
MiniLcm's CRDT backend the single new capability of applying an Motif change set, and let it "carry
...
- **Latency and offline capability.** *Authoring* a change set is pure C#/JSON and needs no LibLCM at all
  ...
  step, which is the step Motif is built to make trustworthy — "author intent → assess the exact state
...
- **Complexity.** Issue B13 — "Cross-process protocol unspecified" — is the honest name for this gap, and
  ...
  non-desktop, or any remote consumer could reach Motif's semantics at all. As of 2026-07-27 it remains
  **open**, and its own description makes the shape of the remaining work explicit: framing, error/exit
  ...
  The only consumer surface that exists is a single-process CLI (`SIL.Motif.Cli`) dispatching twelve
  verbs directly into in-process `Commands` calls over a local files store
  `[VERIFIED — src/SIL.Motif.Cli/Program.cs:1-9, 27-60; build-stages.md:49]`. There is no evidence — not
  a partial implementation, not a spike, not even a chosen framing — of the protocol a client/server split
  would need. This is the largest single gap between "Motif reaches Android" as a sentence and "Motif
...
**Direct answer to the "does this undermine the premise" question:** yes, partially, and Motif's own
newest document says so in different words. Reaching Android is possible only by accepting that the part
of Motif that does the actual work — turning intent into a verified, atomic model change — never runs on
the device, and by building a cross-process protocol that today does not exist even as a chosen design
(B13 is open, not merely unimplemented). What ships to Android under this plan is not Motif; it is a
client that trusts Motif, running elsewhere, to have done its job. Whether that is an acceptable reading
of "Android support is non-negotiable" is a product decision this report cannot make, but it should not be
mistaken for Motif itself running on Android, because it does not and — given the ICU native-dependency
...
`src/SIL.Motif.Runner/Operations/`, which contains exactly four hand-written files
...
## 4. What Motif gets right that alternatives don't
...
- **Semantic operations over raw property mutation closes a real, documented bug class.** The
  ...
  the alternative — `SetPartOfSpeech` silently orphaning an MSA in the tools Motif is meant to replace —
...
- **The Contract layer's engine-neutrality is real, not aspirational, at the code level.** `Contract` and
  ...
  `[VERIFIED — src/SIL.Motif.Contract/SIL.Motif.Contract.csproj`, `src/SIL.Motif.Model/...csproj`, both
...
These are genuine strengths, and they distinguish Motif from "a thin wrapper that calls LibLCM setters" —
which is a fair description of what Flexicon and, per Motif's own documents, MiniLcm's FwData bridge
...
2. The cross-process protocol (B13) has to get built, chosen, and shipped — today it is not even a
   ...
   the AI agents the project is explicitly built for) can reach Motif's semantics at all. This item is
...
4. Someone has to actually resource the project past the walking-skeleton stage. At 42 commits
   ...
   Motif's build stalls indefinitely ... the practical recommendation degrades to 'MiniLcm is what
...
- B13 never gets resolved, or gets resolved in a way (e.g., a heavyweight per-project daemon
  ...
  boundary explicitly disclaims owning `[VERIFIED — README.md:89]` — in which case Motif either grows
...
- The 300-of-473 heuristically-classified manifest rows (issue B18) turn out to contain systematic
  ...
  linguistic dataset) where Motif's entire value proposition is *not* getting this wrong.
...
— Motif's own comparison document makes the same point about the reverse case (MiniLcm's referential
integrity work would not be wasted either) `[VERIFIED — minilcm-evaluation.md:280-293]`. The risk is not
that the work evaporates; it is that "Motif is the one API surface, on every platform" turns out to
...
Motif is a well-designed **change-authorship and review contract** with a genuinely strong correctness
...
native-ICU porting project or accepting that the phone never runs Motif's actual semantics and instead
carries its change sets as opaque payloads to be interpreted somewhere else — a conclusion Motif's own
...
projected from static shape analysis rather than measured from a working generator, "Motif becomes the
one API surface" is best read as "Motif becomes the one change-authorship *contract*, executed on
...
generator exists. The overall verdict is bounded by how much weight to put on Motif's own most recent
...
- Real-world resourcing plans or timelines for Motif past its current walking-skeleton state — this is
